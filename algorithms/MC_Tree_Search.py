...
# TODO cannot be fully random, make random opponent make moves that are close to one another (e.g. try to complete 4s)
def connect4_simulation_heuristic():
    pass
...
def load_functions(game_name):
    parent_dir = dirname(dirname(abspath(__file__)))
    # load module from games folder
    return SourceFileLoader('game', parent_dir + '\\games\\' + game_name + '.py').load_module()

class MCTS():
    def __init__(self, state, epochs, game_name):
        self.epochs = epochs
        self.game = load_functions(game_name)
        self.generate_moves = self.game.generate_moves
        self.modify_state = self.game.modify_state
        self.game_outcome = self.game.game_outcome
        if game_name == 'Connect4':
            self.simulation_heuristic = connect4_simulation_heuristic
        ...
        print(UCB_next_moves)

    def tree_search(self, node, depth):
        # TODO add comments as I go
        moves = self.generate_moves(node.state)
        moves_UCB = []
        for move in moves:
    ...
    def expand_one_node(self, node, move, depth, player):
        ...
        next_state = self.modify_state(deepcopy(node.state), move, player)
        if convert_to_string(self.path) in self.states:
            next_node = self.states[convert_to_string(self.path)]
        else:
        ...
        self.states[convert_to_string(self.path)] = next_node
    ...
    def simulate(self, node):
        ...
        while True:
            human_moves = self.generate_moves(simulation_state)
            if not human_moves:
                return 0
            human_move = choice(human_moves)
            simulation_state = self.modify_state(simulation_state, human_move, self.human)

            ai_moves = self.generate_moves(simulation_state)
            ai_move = choice(ai_moves)
            simulation_state = self.modify_state(simulation_state, ai_move, self.ai)

            outcome = self.game_outcome(simulation_state, abs(self.ai))
...
if __name__ == '__main__':
    ...
    algorithm = MCTS(state, epochs, 'Connect4')
